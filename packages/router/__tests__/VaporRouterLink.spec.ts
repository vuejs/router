...
describe('RouterLink', () => {
  ...
  it('can be active', async () => {
    ...
    expect(wrapper.find('a')?.classList).toContain('router-link-active')
  })
  ...
  it('can customize active class', async () => {
    ...
    expect(wrapper.find('a')!.classList).not.toContain('router-link-active')
    expect(wrapper.find('a')!.classList).toContain('is-active')
  })

  it('prop classes take over global', async () => {
    const to = ref(locations.foo.string)
    const { wrapper, router } = await factory(
      locations.basic.normalized,
    ...
    to.value = locations.basic.string
    await nextTick()
    expect(wrapper.find('a')!.classList).not.toContain('router-link-active')
    expect(wrapper.find('a')!.classList).not.toContain(
      'router-link-exact-active'
    )
    expect(wrapper.find('a')!.classList).not.toContain('custom')
    expect(wrapper.find('a')!.classList).not.toContain('custom-exact')
    expect(wrapper.find('a')!.classList).toContain('is-active')
    expect(wrapper.find('a')!.classList).toContain('is-exact')
  })
  ...
  it('can globally customize active class', async () => {
    ...
    await nextTick()
    expect(wrapper.find('a')!.classList).not.toContain('router-link-active')
    expect(wrapper.find('a')!.classList).toContain('custom')
  })
  ...
  it('can be active with custom class', async () => {
    ...
    expect(wrapper.find('a')!.classList).toContain('router-link-active')
    expect(wrapper.find('a')!.classList).toContain('nav-item')
  })
  ...
  it('is not active on a non matched location', async () => {
    ...
    expect(wrapper.find('a')!.classList).toHaveLength(0)
  })
  ...
  it('can be active as an alias', async () => {
    ...
    expect(wrapper.find('a')!.classList).toContain('router-link-active')
    expect(wrapper.find('a')!.classList).toContain('router-link-exact-active')
    wrapper = (
      await factory(
        locations.alias.normalized,
        { to: locations.basic.string },
  ...
  it('allows adding more click listeners', async () => {
    ...
    const { router, wrapper } = await factory(
      START_LOCATION_NORMALIZED,
      { to: locations.basic.string, onClick: () => onClick },
      locations.basic.normalized
    )
    wrapper.find('a')!.click()
    expect(router.push).toHaveBeenCalledTimes(1)
  ...
  it('allows adding custom classes', async () => {
    ...
    expect(wrapper.find('a')!.classList.toString().split(' ')).toEqual([
      'router-link-active',
      'router-link-exact-active',
      'custom',
      'class',
  ...
  describe('v-slot', () => {
          ...
          default: _slotProps0 => {
            ...
            renderEffect(() =>
              setText(
                x0 as any,
                ' route: ' +
                  JSON.stringify(_slotProps0.route) +
                  ' href: "' +
                  _slotProps0.href +
                  '" isActive: "' +
                  _slotProps0.isActive +
                  '" isExactActive: "' +
                  _slotProps0.isExactActive +
                  '" '
              )
            )
            return n0
    ...
    it('renders an anchor by default', async () => {
      ...
      expect(wrapper.find('a')!.tagName).toBe('A')
      expect(wrapper.find('a')!.childElementCount).toBe(1)
    })
    ...
    it('works with custom directive when custom=true', async () => {
      const Directive: VaporDirective = el => {
        if (el instanceof Node) {
          el.setAttribute('data-test', 'x')
        }
      }
      const AppLink = defineVaporComponent({
        name: 'AppLink',
        setup: () => {
          ...
          withVaporDirectives(n1, [[Directive]])
          return n1
        },
      })
      ...
      expect(wrapper.find('a')!.tagName).toBe('A')
      expect(wrapper.find('a')!.getAttribute('data-test')).toBe('x')
    })
    ...
    describe('Extending RouterLink', () => {
                  ...
                  default: withVaporCtx((slotProps0: any) => {
                    ...
                    renderEffect(() =>
                      setDynamicProps(n6, [
                        attrs,
                        {
                          href: slotProps0.href,
                          class: slotProps0.isActive
                            ? props.activeClass
                            : props.inactiveClass,
                        },
                      ])
      ...
      it('can extend RouterLink with external link', async () => {
        ...
        expect(wrapper.find('a')!.classList).toHaveLength(0)
        expect(wrapper.find('a')!.getAttribute('href')).toEqual(
          'https://esm.dev'
        )
      })
    })
