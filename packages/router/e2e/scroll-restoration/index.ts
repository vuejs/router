...
interface ScrollRestorationPluginOptions {
...
const ScrollRestoration: FunctionPlugin<
...
function useScrollRestoration<Name extends keyof RouteMap = keyof RouteMap>(
...
function _usage() {
  // no scroll? but where, probably not, let the navigation disable scrolling
  useScrollRestoration(false)

  // for virtualized lists
  const scrollEntry = useScrollRestoration()
}
