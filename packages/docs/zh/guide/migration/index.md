...
以前，有一个未被记录的特性允许将重定向记录设置为形如 `/events/:id` 的特殊路径，并且它会复用现有的参数 `id`。现已不再支持，有两个替代选项:

- 使用不包含参数的路由名称：`redirect: { name: 'events' }`。注意，当参数 `:id` 为可选项时将不会生效
- 使用一个函数重建基于目标的新位置：`redirect: to => ({ name: 'events', params: to.params })`

**原因**：这种语法很少使用，是上述版本的 _等效做法_，但是相比之下还不够简短，同时会引入一些复杂性，使路由器变得更重。
